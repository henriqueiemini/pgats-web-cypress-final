...
describe('Automation Exercise E2E Tests - Trabalho Final', () => {
    ...
    describe('Login with Dynamic Account Creation', () => {
        it('Test Case 2: Login User with correct email and password', () => {
            // First, create a new account
            const timestamp = Date.now();
            const testName = `test-pgats-${timestamp}`;
            ...
            // Passo 16: Clicar no botão 'Signup / Login'
            menu.navegarParaLoginSignUp();

            // Passo 17: Verificar que 'Login to your account' está visível
            login.verificarPaginaDeLogin();

            // Passo 18: Inserir email e senha corretos
            login.preencherFormularioDeLogin(testEmail, testPassword);

            // Passo 19: Clicar no botão 'login'
            login.clicarBotaoLogin();

            // Passo 20: Verificar que 'Logged in as username' está visível
            cy.url().should('not.include', '/login');
            cy.contains('a', `Logged in as ${testName}`).should('be.visible');

            // Passo 21: Clicar no botão 'Delete Account'
            menu.deletarConta();

            // Passo 22: Verificar que 'ACCOUNT DELETED!' está visível
            menu.verificarContaDeletada();
        });

    });

    describe('Register and Login/Logout Test Cases (continued)', () => {
        ...
        it('Test Case 4: Logout User', () => {
            ...
            // Now test logout
            // Passo 4: Verificar que 'Logged in as username' está visível
            cy.url().should('not.include', '/login');
            cy.contains('a', `Logged in as ${testName}`).should('be.visible');

            // Passo 5: Clicar no botão 'Logout'
            menu.efetuarLogout();

            // Passo 6: Verificar que o usuário foi navegado para a página de login
            login.verificarPaginaDeLogin();
    ...
    describe('Products Test Cases', () => {
        ...
        it('Test Case 9: Search Product', () => {
            ...
            produtos.buscarProduto(productName);
            ...
            produtos.verificarProdutosRelacionadosABusca(productName);
        });
    ...
    describe('Checkout Tests Cases', () => {
        it('Test Case 15: Place Order - Register before checkout', () => {
            ...
            checkout.inserirComentarioPedido('This is an automated test order for Test Case 15');
